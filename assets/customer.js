  ...
  "use strict";
    ...
    $(document).ready(function() {
        ...
        $('form[action="/cart/add"]').on('submit', function(e) {
            ...
            // 2b. Auto-add the free gift bundle variant, if this PDP has one
            const $giftBundle = $('.gift-bundle');
            const giftVariantId = $giftBundle.data('gift-variant-id');
            if ($giftBundle.length && giftVariantId) {
                items.push({
                    id: giftVariantId,
                    quantity: 1,
                    properties: { '_gift_with_purchase': 'true' }
                });
            }
